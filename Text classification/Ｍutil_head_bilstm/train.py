...
def layer_norm(inputs, epsilon=1e-8):
    # 层归一化
    mean, variance = tf.nn.moments(inputs, [-1], keep_dims=True)  # 将每条样本进行标准化
    normalized = (inputs - mean) / (tf.sqrt(variance + epsilon))  # x - x均值 / 方差
...
class Model:
    def __init__(self, dict_size, dimension_input, dimension_output, seq_len,
                 learning_rate, hidden_size, rnn_layer_size, num_heads=8, attn_windows=range(1, 6), ):
        ...
        with tf.name_scope("build_rnn"):
            ...
            with tf.name_scope('bi_rnn'), tf.variable_scope('bi_rnn'):
                outputs, _, _ = tf.contrib.rnn.static_bidirectional_rnn(fw_cell_m, bw_cell_m, rnn_inputs,
                                                                        dtype=tf.float32)

                # final_output = tf.reshape(tf.concat(outputs, -1), [batch_size, -1, hidden_size])
                final_output = outputs[-1]
                output_layer = tf.layers.dense(final_output, hidden_size, activation=None)

        with tf.name_scope("output"):
            W = tf.get_variable(
                "W",
                shape=[hidden_size, dimension_output],
                initializer=tf.contrib.layers.xavier_initializer())
            b = tf.Variable(tf.constant(0.1, shape=[dimension_output]), name="b")

            self.scores = tf.nn.xw_plus_b(output_layer, W, b, name="scores")
    ...
    def multihead_attn(self, inputs, masks):
        ...
        if masks is not None:   # 加入mask是为了让我模型好算一些
...
model = Model(vocabulary_size+4, embedded_size, dimension_output, maxlen, learning_rate,
              hidden_size=hidden_size, rnn_layer_size=1)
